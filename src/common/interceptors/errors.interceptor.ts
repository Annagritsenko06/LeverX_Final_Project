...
import { USERMESSAGES, VINYLSMESSAGES, MESSAGES } from '../messages';
...
export class ErrorsInterceptor implements NestInterceptor {
  ...
  private handleDatabaseError(error: DatabaseError): never {
    if (
  ...
  private handleApplicationError(error: Error): never {
    if (error instanceof Error) {
      switch (error.message) {
        ...
        case VINYLSMESSAGES.VINYL_NOT_FOUND:
          throw new NotFoundException({ error: error.message });
        default:
